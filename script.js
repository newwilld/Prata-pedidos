...
window.adicionarItensSelecionadosExcluidos = async function() {
    const clientUid = document.getElementById('editClientUid').value;
    ...
    // Obtém os itens marcados atualmente
    const checkedItems = document.querySelectorAll('.item-individual-check:checked');
    ...
    // Categorias dos itens selecionados para exclusão
    const categoriesToExclude = new Set();
    
    checkedItems.forEach(checkbox => {
        const checkboxIndex = Array.from(allCheckboxes).indexOf(checkbox);
        if (checkboxIndex >= 0 && checkboxIndex < allItems.length) {
            categoriesToExclude.add(allItems[checkboxIndex].category);
        }
    });
    ...
    let currentConfig = { allowedItems: ['todos'], excludedItems: [] };
    if (snapshot.exists()) {
        currentConfig = snapshot.val();
    }
    
    // Calcula novos itens permitidos (remove as categorias excluídas)
    let newAllowedItems = [];
    
    if (currentConfig.allowedItems.includes('todos')) {
        // Se era "todos", agora remove as categorias selecionadas
        newAllowedItems = Object.keys(productCatalog).filter(cat => 
            !categoriesToExclude.has(cat)
        );
    } else {
        // Se já tinha categorias específicas, remove as selecionadas
        newAllowedItems = currentConfig.allowedItems.filter(cat => 
            !categoriesToExclude.has(cat)
        );
    }
    
    // Se todas as categorias foram excluídas, mantém pelo menos uma
    if (newAllowedItems.length === 0) {
        showToast('Não é possível excluir todos os itens. Mantenha pelo menos uma categoria.', 'error');
        return;
    }
    
    const itemsConfig = {
        allowedItems: newAllowedItems,
        excludedItems: Array.from(categoriesToExclude)
    };
    ...
    try {
        ...
        // Atualiza checkboxes principais
        document.getElementById('itemsAllowedTodos').checked = false;
        document.getElementById('itemsAllowedPizza').checked = newAllowedItems.includes('Caixa de pizza');
        document.getElementById('itemsAllowedTorta').checked = newAllowedItems.includes('Caixa de torta');
        document.getElementById('itemsAllowedCorreio').checked = newAllowedItems.includes('Caixa correio');
        
        // Atualiza display
        updateItemsConfigDisplay(itemsConfig);
        
        // Atualiza visualmente os checkboxes individuais
        allCheckboxes.forEach((checkbox, index) => {
            if (index < allItems.length) {
                const category = allItems[index].category;
                if (newAllowedItems.includes(category)) {
        ...
        showToast('Itens adicionados à lista de excluídos com sucesso!', 'success');
        ...
        // Não esconde a lista para que o admin possa ver as alterações
